...
const EPISODE_BRIEF_PATH: &str = r"E:\codex\hope\tests\e2e\30min-single-episode\episode-brief.json";
const SYNOPSIS_PATH: &str = r"E:\codex\hope\tests\e2e\30min-single-episode\synopsis.json";
const STORY_PATH: &str = r"E:\codex\hope\tests\e2e\30min-single-episode\story.json";
const SCREENPLAY_PATH: &str = r"E:\codex\hope\tests\e2e\30min-single-episode\screenplay.json";

fn suspense_dialogue_taxonomy() -> storyboard_pipeline::SceneTaxonomyRecord {
    storyboard_pipeline::SceneTaxonomyRecord {
        scene_taxonomy_id: "scene-taxonomy-suspense-dialogue".to_string(),
        scene_type: "suspense_dialogue".to_string(),
        display_name: "悬疑对白".to_string(),
        definition: "带有信息差和心理压力的对白推进段落。".to_string(),
        ...
        risk_flags: vec!["continuity_break".to_string()],
        continuity_priority: "high".to_string(),
        prompt_focus: vec!["信息差".to_string(), "悬念推进".to_string()],
...
#[test]
fn benchmark_30min_single_episode_runs_end_to_end() {
    write_fixture(
        "30min single episode",
        30,
        &[EpisodeSpec {
    ...
    assert_eq!(episode_brief["episode_duration_minutes"].as_u64(), Some(30));
    assert!(synopsis.document.json.contains("30"));
    assert_eq!(
        story_input
            .duration_policy
    ...
    assert_eq!(fixture.narrative_scene.len(), 9);
    assert_eq!(fixture.render_segment.len(), 27);
    assert_eq!(fixture.cut.len(), 81);
    assert_eq!(fixture.handoff_zone.len(), 27);
    assert_eq!(fixture.prompt_package.len(), 54);
    assert_eq!(fixture.stale_event.len(), 27);
    ...
    let total_segment_seconds: u32 = fixture
        ...
        .sum();
    assert_eq!(total_segment_seconds, 1800);
    ...
    for render_segment in &fixture.render_segment {
        ...
        let plan = build_storyboard_plan(StoryboardPlanRequest {
            ...
            scene_taxonomy: Some(suspense_dialogue_taxonomy()),
    ...
    assert_eq!(validation.validation_report.len(), 217);
    ...
    assert_eq!(export_json["cut"].as_array().expect("cut").len(), 81);
    assert_eq!(
